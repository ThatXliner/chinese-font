...
from hardcode import CHAR_GLOSSES, LIGATURES
...
def parse_cedict(
    ...
    combined_ligatures = dict(LIGATURES)
    for chars, gloss in ligatures.items():
        combined_ligatures.setdefault(chars, gloss)

    return {**single_chars, **CHAR_GLOSSES}, combined_ligatures
...
def build_gsub_table(cmap: dict, ligature_glyph_map: dict):
    ...
    sorted_ligatures = sorted(
        ligature_glyph_map.items(),
        key=lambda item: (-len(item[0]), item[0]),
    )

    for chars, lig_glyph in sorted_ligatures:
        first_char = chars[0]
        rest_chars = chars[1:]
